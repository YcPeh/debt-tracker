...
export const UserTransaction = (imageSrc) => {
  ...
  const handleDelete = async (customId) => {
    try {
      ...
      await axios.delete(`/api/userTransaction/${customId}`);
  ...
  const handleHome = () => {
    dispatch(
      loadLineChart({
        userNameCustomIdForLineChart: userNameCustomId,
                ...
                <Accordion.Item eventKey={trans.customId}>
                  <Row>
                    ...
                    >
                      {/* <DeleteButton/> */}
                      <DeleteButton
                        handleDelete={() => handleDelete(trans.customId)}
                        key={trans.customId}
                      />
                    </Col>
                    <Col
                    ...
                    <Col xs={{ span: 9, offset: 0 }}>
                      <Accordion.Header>
                        <Col
                          className="p-0 d-flex justify-content-center text-center text-wrapper"
                          xs={{ span: 5, offset: 0 }}
                        >
                          {trans.title}
                        </Col>
                        ...
                        <Col
                          className="p-0 d-flex justify-content-end text-wrapper"
                          xs={{ span: 3, offset: 0 }}
                        >
                  ...
                  <Accordion.Body>
                    {bodyInfo.map((info, innerIndex) => (
                      <Row key={innerIndex}>
                        <Col
                          className="p-0 d-flex justify-content-start text-wrapper"
                          xs={{ span: 3, offset: 1 }}
                        >
                          {info.label}
                        </Col>
  ...
  return (
    <Container fluid className="user-transaction-container">
      ...
      <Row>
        ...
        <Col xs={{ span: 3, offset: 0 }}>
          <RenderBarChart
            totalDebt={debtRepayment.totalDebtRM}
            remainingDebt={
              debtRepayment.totalDebtRM - debtRepayment.totalRepayRM
            }
            labels="Debts (RM)"
            currency={"RM"}
          />
        ...
        <Col xs={{ span: 3, offset: 0 }}>
          <RenderBarChart
            totalDebt={debtRepayment.totalDebtTHB}
            remainingDebt={
              debtRepayment.totalDebtTHB - debtRepayment.totalRepayTHB
            }
            labels="Debts (THB)"
            currency={"THB"}
          />
      ...
      <div className="user-transaction-content">
        <section className="left-section">
          <Row className="UserTransactionTitles">
            ...
            <AddButton
              colWidthAddButton={2}
              divClassName={"divContainerAddButtonTransaction"}
              linkToRoute={"/userTransaction/userTransactionForm"}
              propsToPass={{
                category: "Debts",
                userNameCustomId: customId,
                userName: name,
          ...
          <Row>{handleTransaction("Debts")}</Row>
        </section>
        ...
        <section className="right-section">
          ...
          <Row>{handleTransaction("Repayments")}</Row>
        </section>
      </div>
    </Container>
  );
