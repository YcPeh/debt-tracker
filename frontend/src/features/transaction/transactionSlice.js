...
const getSortedDateTransaction = (transaction, userNameCustomId, currency) => {
  const trimmedData = transaction
    .filter(
      (trans) =>
        trans.userNameCustomId === userNameCustomId &&
...
const getSingleDate = (data) => {
  ...
  data.forEach((item, index) => {
    ...
    if (
      index >= 1 &&
      result[index - numOfDateSame - 1].date ===
        result[index - numOfDateSame].date
    ) {
  ...
  reducers: {
    ...
    deleteTransaction: (state, action) => {
      ...
      state.transaction = state.transaction.filter(
        (trans) => trans.customId !== idToRemove
      );
    },
    ...
    toggleLineChartCurency: (state, action) => {
      state.currencyForLineChart =
        state.currencyForLineChart === "THB" ? "RM" : "THB";
    ...
    loadLineChart: (state, action) => {
      ...
      if (
        ...
        action.payload.userNameCustomIdForLineChart === "" ||
        action.payload.userNameForLineChart === ""
      ) {
      ...
      // const currency = 'THB';
      const sortedDateTransaction = getSortedDateTransaction(
        state.transaction,
        userNameCustomIdForLineChart,
        state.currencyForLineChart
      );
      ...
      const sortedDateTransactionWithNettAmount = getNettAmount(
        sortedDateTransaction
      );
      const sortedSingleDateTransactionWithNettAmount = getSingleDate(
        sortedDateTransactionWithNettAmount
      );
      state.sortedSingleDateTransactionWithNettAmount =
        sortedSingleDateTransactionWithNettAmount;
