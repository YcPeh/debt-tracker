...
export const updateTransaction = async (req, res, next) => {
  try {
    ...
    const { title, date, category, type, currency, amount, description } =
      req.body;
    const updatedUser = await transactionModel.findOneAndUpdate(
      { customId: idFromFrontEnd },
      { title, date, category, type, currency, amount, description },
      { new: true }
    );
    if (!updatedUser) {
      return res.status(404).json({
